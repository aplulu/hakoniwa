...
export function CreateInstanceView({
  ...
  const canUsePersistent =
    enablePersistence &&
    user?.type === 'openid_connect' &&
    (selectedType?.persistable ?? false);

  const getPersistentDisabledReason = () => {
    if (!enablePersistence)
      return t('workspace.create.persistent_disabled_global');
    if (user?.type !== 'openid_connect')
      return t('workspace.create.persistent_disabled_hint');
    if (selectedType && !selectedType.persistable)
      return t('workspace.create.persistent_disabled_type');
    return t('workspace.create.persistent_disabled_hint');
  };
      ...
      <Flex direction="column" gap="4">
        ...
        <Box>
          <Heading
            size="8"
            weight="bold"
            style={{ letterSpacing: '-0.03em', marginBottom: '12px' }}>
            {t('workspace.create.modal_title')}
          </Heading>
          <Text size="3" color="gray">
            {t('workspace.create.modal_desc')}
          </Text>
        </Box>
      </Flex>
                ...
                <Flex asChild align="center" gap="2">
                  <label
                    ...
                    <Flex gap="1" align="center">
                      <HardDrive size={16} />
                      <Text size="2">
                        {t('workspace.create.enable_persistent')}
                      </Text>
                    </Flex>
                  </label>
                </Flex>
                ...
                <Box style={{ opacity: 0.5 }}>
                  <Flex asChild align="center" gap="2">
                    <label
                      ...
                      <Checkbox checked={persistent} disabled={true} />
                      <Flex gap="1" align="center">
                        <HardDrive size={16} />
                        <Text size="2">
                          {t('workspace.create.enable_persistent')}
